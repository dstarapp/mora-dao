...
URL="https://github.com/dstarapp/mora-dao/releases/tag/$TAG"
...
https://github.com/dstarapp/mora-dao/commit/$COMMIT_ID
...
./scritps/build.sh $CANISTER_NAME
\`\`\`"
...
if [ "$FUNCTION_ID" -ge "1000" ] ; then
    ...
    # Submit the proposal
    ./sns/submit_upgrade_proposal.sh $CANISTER_NAME $VERSION "$TITLE" "$URL" "$SUMMARY"
fi
